...
export default function BookCard({ book }) {
  ...
  const wished = useSelector((state) => {
    ...
    return items.some((item) => item._id === book?._id);
  });
  ...
  return (
    <article className="group bg-plum-50 dark:bg-plum-900/30 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <p className="text-xs text-plum-500 dark:text-plum-400 mt-1 mb-3">
          {book.author}
        </p>
        ...
        <div className="mt-auto flex items-center justify-between pt-2">
          <div className="flex items-baseline gap-2">
            <span className="text-base font-bold text-plum-900 dark:text-plum-100">
              ${price.toFixed(2)}
            </span>

            {discountPrice > 0 && (
              <span className="text-xs text-plum-400 dark:text-plum-500 line-through">
